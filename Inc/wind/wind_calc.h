 ...
 * UPDATED: Extended Phase Unwrapping for 75 m/s wind speed targets.
 */
...
bool calculate_wind(int16_t *x_cmps, int16_t *y_cmps) {
    ...
    // Phase Unwrapping Esteso per target 75 m/s
    for (int8_t ch2W = -3; ch2W <= 3; ch2W++) {
        for (int8_t ch3W = -3; ch3W <= 3; ch3W++) {

            // Assi Corti: gestiscono l'aliasing per venti oltre 46 m/s
            for (int8_t ch04W = -1; ch04W <= 1; ch04W++) {
                for (int8_t ch15W = -1; ch15W <= 1; ch15W++) {
                    ...
                    for (uint8_t ch = 0; ch < 6; ch++) {
                        ...
                        // Applicazione dei correttivi di fase
